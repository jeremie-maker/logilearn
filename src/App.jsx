...
const CH10 = {id:"c10",num:10,title:"Les modes de transport",icon:"🚛",color:"#37474F",textColor:"#fff",
 content:[
  ...
  {T:"section",x:"Le Traité de Maastricht (1992) et le rail"},
  {T:"hl",bg:"#e3f2fd",b:"#1565C0",ti:"Traité de Maastricht — Impact sur le transport ferroviaire",x:"Le traité de Maastricht (1992) a jeté les bases d'une nouvelle dimension politique européenne sur trois piliers : la Communauté européenne / une politique étrangère et de sécurité commune / la collaboration justice-police.\n\nPour le transport ferroviaire, ce traité a permis d'UNIFIER les locomotives, les wagons et le personnel pour rouler à l'international, facilitant le transit de marchandises entre pays européens sans changer d'engin en frontière."},
  {T:"section",x:"Les compagnies ferroviaires suisses"},
  {T:"list",items:["CFF Cargo : plus grand prestataire de fret ferroviaire en Suisse, filiale des CFF. Dispose de ses propres locomotives et personnel.","ChemOil Logistic : filiale de CFF Cargo spécialisée dans le transport de pétrole et de produits chimiques.","BLS Cargo (créé en 2001) : filiale des BLS (Bern-Lötschberg-Simplon-Bahnen). Spécialisée dans le transit alpin via Lötschberg, Simplon et Gothard.","RHB (Chemins de Fer Rhétiques) : voie étroite (< 1435 mm). Dessert la majeure partie du canton des Grisons."]},
  {T:"section",x:"Les types de wagons de marchandises"},
  {T:"list",items:["Wagon couvert : protection contre intempéries pour marchandises sensibles","Wagon spécial couvert : pour marchandises nécessitant conditions particulières","Wagon plat : transport de marchandises non sensibles aux intempéries","Wagon ouvert : minerais, graviers, produits en vrac","Wagon silo : produits en vrac (céréales, ciment, produits chimiques)","Wagon plate-forme à quatre essieux : marchandises lourdes et volumineuses"]},
  {T:"hl",bg:"#e3f2fd",b:"#1565C0",ti:"Réseau ferroviaire suisse — Chiffres clés",x:"5'000 km de rails (réseau le plus dense d'Europe)\n3'000 km exploités par les CFF, le reste par des compagnies privées\n671 tunnels (dont 250 CFF) / 6'000 ponts ferroviaires\nNLFA : Nouvelles Lignes Ferroviaires à travers les Alpes (tunnels de base Gothard et Lötschberg)\nTrafic transalpin en Europe : se répartit entre France, Autriche et Suisse. La Suisse est le leader pour le déplacement par rail."},
  ...
  {T:"gloss",items:[
    ...
    {term:"Traité de Maastricht (1992)",def:"Traité européen fondateur (entré en vigueur en 1993) reposant sur 3 piliers : la Communauté européenne, la politique étrangère commune, la coopération justice/police. Pour le transport ferroviaire : il a permis d'UNIFIER locomotives, wagons et personnel entre pays membres, facilitant le transit international de marchandises sur rail."},
    {term:"ChemOil Logistic",def:"Filiale de CFF Cargo spécialisée dans le transport ferroviaire de pétrole et de produits chimiques. Opère sur le réseau suisse pour le transport de ces matières qui nécessitent des wagons et procédures spéciaux."},
    {term:"LIFO maritime",def:"Les conteneurs sur les navires sont chargés selon le principe LIFO (Last In, First Out) : le dernier chargé est le premier déchargé. Un OFFICIER DE CHARGEMENT est responsable du chargement et de l'équilibrage du navire pour garantir sa stabilité."},
    {term:"Types de wagons de marchandises",def:"Wagon couvert (marchandises sensibles aux intempéries), wagon spécial couvert, wagon plat (marchandises résistantes), wagon ouvert (vrac, minerais), wagon silo (céréales, ciment), wagon plate-forme à 4 essieux (charges lourdes et volumineuses)."},
  ...
  function renderQuestion() {
    ...
    return (
      <div>
        ...
        {/* Cours button during quiz */}
        <div style={{display:"flex",justifyContent:"flex-end",marginBottom:"8px"}}>
          <button onClick={pauseForCourse} style={{background:"#f1f5f9",border:"1px solid #cbd5e1",borderRadius:"6px",padding:"4px 10px",cursor:"pointer",fontSize:"11px",fontWeight:"700",color:"#475569"}}>📖 Cours</button>
        </div>
  ...
  return (
    <div style={{maxWidth:780,margin:"0 auto",padding:"14px"}}>
      <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"12px",flexWrap:"wrap"}}>
        <button onClick={onBack} style={{background:"#f3f4f6",border:"none",borderRadius:"7px",padding:"5px 11px",cursor:"pointer",fontSize:"11px",fontWeight:"600",color:"#374151"}}>← Module C</button>
        <div>
      ...
      {view !== "quiz" && view !== "result" && (
        <div style={{display:"flex",gap:"7px",marginBottom:"14px"}}>
          <button onClick={() => setView("content")} style={{padding:"7px 14px",borderRadius:"7px",border:"none",background:view==="content"?ch.color:"#f3f4f6",color:view==="content"?ch.textColor:"#374151",fontWeight:"700",cursor:"pointer",fontSize:"12px"}}>📖 Cours</button>
          <button onClick={() => setView("glossaire")} style={{padding:"7px 14px",borderRadius:"7px",border:"none",background:view==="glossaire"?"#0369a1":"#f3f4f6",color:view==="glossaire"?"#fff":"#374151",fontWeight:"700",cursor:"pointer",fontSize:"12px"}}>📚 Glossaire</button>
          <button onClick={quizPaused ? resumeQuiz : startQuiz} style={{padding:"7px 14px",borderRadius:"7px",border:"none",background:"#f3f4f6",color:"#374151",fontWeight:"700",cursor:"pointer",fontSize:"12px"}}>
            {quizPaused ? `▶ Reprendre QCM (Q${currentQ+1}/${ch.questions.length})` : `🧠 QCM (${ch.questions.length})`}
          </button>
        </div>
      )}
      ...
      {view === "content" && (
        <div style={{background:"#fff",borderRadius:"12px",padding:"16px",border:"1px solid #e5e7eb"}}>
          ...
          <div style={{borderTop:"1px solid #e5e7eb",marginTop:"16px",paddingTop:"14px",display:"flex",gap:"8px",flexWrap:"wrap"}}>
            <button onClick={() => setView("glossaire")} style={{flex:1,padding:"9px",borderRadius:"8px",border:"1px solid #7dd3fc",background:"#f0f9ff",color:"#0369a1",fontWeight:"700",cursor:"pointer",fontSize:"12px"}}>📚 Glossaire</button>
            {quizPaused
              ? <button onClick={resumeQuiz} style={{flex:2,padding:"9px",borderRadius:"8px",border:"none",background:"#7c3aed",color:"#fff",fontWeight:"800",cursor:"pointer",fontSize:"12px"}}>▶ Reprendre le QCM (Q{currentQ+1}/{ch.questions.length})</button>
      ...
      {view === "glossaire" && (
        <div style={{background:"#fff",borderRadius:"12px",padding:"16px",border:"1px solid #e5e7eb"}}>
          ...
          <div style={{borderTop:"1px solid #e5e7eb",marginTop:"13px",paddingTop:"12px",display:"flex",gap:"8px"}}>
            <button onClick={() => setView("content")} style={{flex:1,padding:"9px",borderRadius:"8px",border:`2px solid ${ch.color}`,background:"#fff",color:"#111",fontWeight:"700",cursor:"pointer",fontSize:"12px"}}>📖 Cours</button>
            {quizPaused
              ? <button onClick={resumeQuiz} style={{flex:2,padding:"9px",borderRadius:"8px",border:"none",background:"#7c3aed",color:"#fff",fontWeight:"800",cursor:"pointer",fontSize:"12px"}}>▶ Reprendre le QCM</button>
              : <button onClick={startQuiz} style={{flex:2,padding:"9px",borderRadius:"8px",border:"none",background:ch.color,color:ch.textColor,fontWeight:"800",cursor:"pointer",fontSize:"12px"}}>QCM ({ch.questions.length} questions) →</button>
            }
          </div>
        </div>
      )}
      ...
      {view === "result" && (
        <div>
          ...
          <div style={{display:"flex",gap:"7px",flexWrap:"wrap"}}>
            <button onClick={() => setView("content")} style={{flex:1,padding:"9px",borderRadius:"8px",border:`2px solid ${ch.color}`,background:"#fff",color:"#111",fontWeight:"700",cursor:"pointer",fontSize:"11px"}}>📖 Cours</button>
            <button onClick={() => setView("glossaire")} style={{flex:1,padding:"9px",borderRadius:"8px",border:"1px solid #7dd3fc",background:"#f0f9ff",color:"#0369a1",fontWeight:"700",cursor:"pointer",fontSize:"11px"}}>📚 Glossaire</button>
            <button onClick={startQuiz} style={{flex:1,padding:"9px",borderRadius:"8px",border:"none",background:ch.color,color:ch.textColor,fontWeight:"700",cursor:"pointer",fontSize:"11px"}}>🔁 Refaire</button>
            <button onClick={onBack} style={{flex:1,padding:"9px",borderRadius:"8px",border:"none",background:"#37474F",color:"#fff",fontWeight:"700",cursor:"pointer",fontSize:"11px"}}>← Module C</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#f8fafc 0%,#eff6ff 100%)",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <div style={{background:"#fff",borderBottom:"1px solid #e5e7eb",position:"sticky",top:0,zIndex:20,boxShadow:"0 1px 5px rgba(0,0,0,0.07)"}}>
        <div style={{display:"flex",alignItems:"center",gap:"9px",padding:"7px 14px",borderBottom:"1px solid #f1f5f9"}}>
        ...
        <div style={{display:"flex",gap:"2px",padding:"0 11px",overflowX:"auto"}}>
          ...
          {(activeModule||activeChapter||showRandom)&&<button onClick={()=>{setActiveModule(null);setActiveChapter(null);setShowRandom(false);}} style={{padding:"7px 11px",border:"none",background:"transparent",cursor:"pointer",fontWeight:"600",fontSize:"11px",color:"#9ca3af",borderBottom:"2.5px solid transparent"}}>← Retour</button>}
        </div>
      </div>
