...
export default function App() {
  ...
  // ── NAVIGATION ROBUSTE : pushState + hashchange + popstate ──────────────────
  // Format hash: home | mod-A | ch-A-ca1-content | ch-A-ca1-quiz | rand-A
  ...
  useEffect(()=>{
    // Au chargement : enregistrer l'état initial dans l'historique via replaceState
    // Cela garantit que le bouton retour peut revenir à cet état depuis l'intérieur de l'app
    const initHash = window.location.hash.replace(/^#/,"") || "home";
    window.history.replaceState({hash:initHash}, "", "#"+initHash);

    // Synchroniser le state React à chaque navigation historique (←/→)
    const sync = ()=>setNav(parseHash(window.location.hash));
  ...
  // pushState crée une entrée dans l'historique ET met à jour React immédiatement
  const go = (hash) => {
    ...
    const goHome    = ()         => { const h=window.location.hash; if(h===""||h==="#home")return; go("home"); };
