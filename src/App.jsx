...
export default function App() {
  ...
  // Hash format: #home | #mod-A | #ch-A-ca1-content | #ch-A-ca1-glossaire | #ch-A-ca1-quiz | #rand-A
  function parseHash(hash) {
    ...
    if (h.startsWith("ch-")) {
      const parts = h.split("-"); // ["ch","A","ca1"] or ["ch","A","ca1","content"]
  ...
  useEffect(()=>{
    const onPop = ()=>setNav(parseHash(window.location.hash));
  ...
  const goHome    = ()        => go("home",                          {mod:null,  ch:null,    tab:"content", rand:false});
  const goModule  = (k)       => go(`mod-${k}`,                     {mod:k,     ch:null,    tab:"content", rand:false});
  const goChapter = (m,chId)  => go(`ch-${m}-${chId}`,             {mod:m,     ch:chId,    tab:"content", rand:false});
  const goTab     = (m,chId,t)=> go(`ch-${m}-${chId}-${t}`,        {mod:m,     ch:chId,    tab:t,         rand:false});
  const goRandom  = (m)       => go(`rand-${m}`,                    {mod:m,     ch:null,    tab:"content", rand:true});
