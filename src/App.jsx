...
export default function App() {
  ...
  const [activeModule, setActiveModule] = useState(null); // "A"|"B"|"C"|"D"|"E"
  const [activeChapter, setActiveChapter] = useState(null); // chapter.id
  ...
  const goHome = () => { setActiveModule(null); setActiveChapter(null); setShowRandom(false); };
  const goModule = (k) => { setActiveModule(k); setActiveChapter(null); setShowRandom(false); };
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#f8fafc 0%,#eff6ff 100%)",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      ...
      {activeModule&&!activeChapter&&!showRandom&&<ModuleDashboard modKey={activeModule} scores={allScores[activeModule]||{}} randScores={allRandScores[activeModule]||[]} onSelectChapter={id=>setActiveChapter(id)} onStartRandom={()=>setShowRandom(true)}/>}
